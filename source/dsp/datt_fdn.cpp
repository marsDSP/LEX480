#ifndef LEX480_DATT_FDN_CPP
#define LEX480_DATT_FDN_CPP
...
namespace MarsDSP::Reverb
{
    ...
    {
        Math::MatrixOps::HouseHolder<FloatType, nChannels>::inPlace(data);
    }

    template<typename FloatType, int nChannels, typename StorageType>
    FloatType DefaultFDNConfig<FloatType, nChannels, StorageType>::calcGainForT60(
        FloatType decayTimeMs, FloatType delayTimeMs)
    {
    ...
    void DefaultFDNConfig<FloatType, nChannels, StorageType>::setDecayTimeMs(
        const FDNType &fdn, FloatType decayTimeLowMs, FloatType decayTimeHighMs, FloatType crossoverFreqHz)
    {
    ...
    template<typename FloatType, int nChannels, typename StorageType>
    const FloatType *DefaultFDNConfig<FloatType, nChannels, StorageType>::doFeedbackProcess(
        DefaultFDNConfig &fdnConfig, const FloatType *data)
    {
        for (size_t i = 0; i < (size_t) nChannels; ++i)
    ...
    void FDN<FDNConfig, DelayInterpType, delayBufferSize>::setDelayTimeMsWithModulators(
        FloatType delayTimeMs, FloatType (&modulationAmt)[NModulators])
    {
    ...
    template<typename FDNConfig, typename DelayInterpType, int delayBufferSize>
    typename FDN<FDNConfig, DelayInterpType, delayBufferSize>::FloatType FDN<FDNConfig, DelayInterpType,
        delayBufferSize>::getChannelDelayMs(size_t channelIndex) const noexcept
    {
