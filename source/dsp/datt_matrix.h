...
#ifndef LEX480_DATT_MATRIX_H
#define LEX480_DATT_MATRIX_H
...
namespace MarsDSP::Math
{
    ...
    inline namespace MatrixOps
    {
        ...
        private:
            ...
            static constexpr NumericType multiplier =
                    static_cast<NumericType>(-2) / static_cast<NumericType>(size * Utils::TypeTraits<FloatType>::Size);
        ...
        public:
            ...
            outOfPlace(FloatType *out, const FloatType *in)
            {
                ...
                static constexpr auto vec_size = static_cast<int>(Vec::size);
            ...
            template<typename T = FloatType>
            static std::enable_if_t<SampleTypeHelpers::IsSIMDRegister<T>, void>
            outOfPlace(FloatType *out, const FloatType *in)
            {
                FloatType vecSum{};
            ...
            static void inPlace(FloatType *arr)
            {
                outOfPlace(arr, arr);
            }
        };
        ...
        private:
            using NumericType = SampleTypeHelpers::NumericType<FloatType>;
            static constexpr NumericType scalingFactor = gcem::sqrt(
                static_cast<NumericType>(1) / NumericType(size * Utils::TypeTraits<FloatType>::Size));
            static_assert(Math::isPowerOfTwo(size * Utils::TypeTraits<FloatType>::Size),
                          "Hadamard matrix dimension must be a power of 2!");

        public:
            /** Perform unscaled Hadamard transformation using recursion */
            template<typename T = FloatType>
            static std::enable_if_t<std::is_floating_point_v<T> || (size > 1), void>
            ...
            template<typename T = FloatType>
            static std::enable_if_t<SampleTypeHelpers::IsSIMDRegister<T> && size == 1, void>
            recursiveUnscaled(FloatType *out, const FloatType *in)
            {
            ...
            inPlace(FloatType *arr)
            {
                ...
                {
                    Vec arrVec[static_cast<size_t>(size / vec_size)]{};
