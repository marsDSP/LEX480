...
namespace MarsDSP::Reverb
{
    ...
    ConvolutionEngine<FFTEngineType>::ConvolutionEngine(size_t numSamples, size_t maxBlockSize, const float *initialIR)
        : irNumSamples(numSamples),
          blockSize(static_cast<size_t>(juce::nextPowerOfTwo(static_cast<int>(maxBlockSize)))),
          fftSize(blockSize > 128 ? 2 * blockSize : 4 * blockSize),
          fftObject(std::make_unique<FFTEngineType>(log2(fftSize))),
    ...
    template<typename FFTEngineType>
    ConvolutionEngine<
        FFTEngineType>::ConvolutionEngine(ConvolutionEngine &&other) noexcept : irNumSamples(other.irNumSamples),
    ...
    template<typename FFTEngineType>
    void ConvolutionEngine<FFTEngineType>::updateSegmentsIfNecessary(size_t numSegmentsToUpdate,
                                                                     std::vector<juce::AudioBuffer<float> > &segments,
                                                                     size_t fftSize)
    {
        if (numSegmentsToUpdate == 0
            || numSegmentsToUpdate != segments.size()
            || static_cast<size_t>(segments[0].getNumSamples()) != fftSize * 2)
        {
            segments.clear();
        ...
        for (auto &buf: buffersImpulseSegments)
        {
            ...
            juce::FloatVectorOperations::copy(impulseResponse,
                                              ...
                                              static_cast<int>(juce::jmin(
                                                  fftSize - blockSize, irNumSamples - currentPtr)));
    ...
    void ConvolutionEngine<FFTEngineType>::processSamples(const float *input, float *output, size_t numSamples)
    {
        ...
        while (numSamplesProcessed < numSamples)
        {
            ...
            juce::FloatVectorOperations::copy(inputData + inputDataPos, input + numSamplesProcessed,
            ...
            // Add overlap
            juce::FloatVectorOperations::add(&output[numSamplesProcessed], &outputData[inputDataPos],
                                             &overlapData[inputDataPos], static_cast<int>(numSamplesToProcess));
            ...
            if (inputDataPos == blockSize)
            {
                ...
                juce::FloatVectorOperations::add(&(outputData[blockSize]), &(overlapData[blockSize]),
                                                 static_cast<int>(fftSize - 2 * blockSize));

                // Save the overlap
                juce::FloatVectorOperations::copy(overlapData, &(outputData[blockSize]),
    ...
    template<typename FFTEngineType>
    void ConvolutionEngine<FFTEngineType>::processSamplesWithAddedLatency(
        const float *input, float *output, size_t numSamples)
    {
        // Overlap-add, zero latency convolution algorithm with uniform partitioning
        size_t numSamplesProcessed = 0;
        ...
        while (numSamplesProcessed < numSamples)
        {
            ...
            juce::FloatVectorOperations::copy(output + numSamplesProcessed, outputData + inputDataPos,
                                              static_cast<int>(numSamplesToProcess));
    ...
    template<typename FFTEngineType>
    void ConvolutionEngine<FFTEngineType>::convolutionProcessingAndAccumulate(
        const float *input, const float *impulse, float *output) const
    {
        auto FFTSizeDiv2 = fftSize / 2;

        juce::FloatVectorOperations::addWithMultiply(output, input, impulse, static_cast<int>(FFTSizeDiv2));
        juce::FloatVectorOperations::subtractWithMultiply(output, &(input[FFTSizeDiv2]), &(impulse[FFTSizeDiv2]),
                                                          static_cast<int>(FFTSizeDiv2));

        juce::FloatVectorOperations::addWithMultiply(&(output[FFTSizeDiv2]), input, &(impulse[FFTSizeDiv2]),
                                                     static_cast<int>(FFTSizeDiv2));
        juce::FloatVectorOperations::addWithMultiply(&(output[FFTSizeDiv2]), &(input[FFTSizeDiv2]), impulse,
                                                     static_cast<int>(FFTSizeDiv2));
