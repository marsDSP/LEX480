#ifndef LEX480_BUFFER_MATH_CPP
#define LEX480_BUFFER_MATH_CPP

#include "buffer_math.h"

namespace MarsDSP::Math::BufferMath
{
    ...
    auto getRMSLevel (const BufferType& buffer, int channel, int startSample, int numSamples) noexcept
    {
        ...
        else if constexpr (SampleTypeHelpers::IsSIMDRegister<SampleType>)
        {
            ...
            return xsimd::sqrt (sum / static_cast<NumericType>(numSamples));
        }
    #endif
    }
    ...
    void sumToMono (const BufferType1& bufferSrc, BufferType2& bufferDest, FloatType normGain)
    {
        ...
        if (normGain < static_cast<FloatType>(0))
            normGain = static_cast<FloatType>(1) / static_cast<FloatType>(bufferSrc.getNumChannels());
        ...
        applyFunctionSIMD (const BufferType1& bufferSrc, BufferType2& bufferDest, SIMDFunctionType&& simdFunction, ScalarFunctionType&& scalarFunction) noexcept
    {
        ...
        static constexpr auto vecSize = static_cast<int> (xsimd::batch<FloatType>::size);
        ...
        for (int ch = 0; ch < numChannels; ++ch)
        {
            ...
            {
                xsimd::store_unaligned (dataOut, simdFunction (xsimd::load_unaligned (dataIn)));
                dataIn += vecSize;
                dataOut += vecSize;
            }
...
#endif // MEMORYBOY_BUFFER_MATH_CPP
