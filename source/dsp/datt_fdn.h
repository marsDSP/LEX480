...
#ifndef LEX480_DATT_FDN_H
#define LEX480_DATT_FDN_H

#include <JuceHeader.h>
#include "datt_filters.h"

#include "datt_diffusor.h"
#include "datt_matrix.h"
...
namespace MarsDSP::Reverb
{
    ...
    {
        virtual ~DefaultFDNConfig() = default;
        ...
        void setDecayTimeMs(const FDNType &fdn, FloatType decayTimeLowMs, FloatType decayTimeHighMs,
                            FloatType crossoverFreqHz);
...
#else
        std::array<FloatType, static_cast<size_t>(nChannels)> fbData;
#endif

    private:
        std::array<ShelfFilter<FloatType>, static_cast<size_t>(nChannels)> shelfs;
        FloatType fs = 48000.0f;
    };

    /** A feedback delay network processor with a customizable configuration */
    template<typename FDNConfig, typename DelayInterpType = Math::DelayLineInterpolationTypes::None, int delayBufferSize =
            1 << 18>
    class FDN
    {
        static_assert(Utils::types_list::TypesList<Math::DelayLineInterpolationTypes::None,
                          Math::DelayLineInterpolationTypes::Linear>::
                      contains<DelayInterpType>,
                      "DelayInterpType must be either None or Linear");

        using FloatType = FDNConfig::Float;
        static constexpr auto nChannels = FDNConfig::NChannels;

        using DelayType = Buffers::StaticDelayBuffer<FloatType, DelayInterpType, delayBufferSize, typename
            FDNConfig::FloatStorageType>;
    ...
    private:
        ...
        std::array<FloatType, static_cast<size_t>(nChannels)> delayTimesSamples;
