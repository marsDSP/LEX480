...
#ifndef LEX480_DATT_INPUT_NETWORK_H
#define LEX480_DATT_INPUT_NETWORK_H
...
namespace MarsDSP::Reverb
{
    ...
    struct DefaultInputNetworkConfig
    {
        ...
        static int getInputDiffusionTimeSamples(InputDiffusionStage diffusionStage, Float fs)
        {
            if (diffusionStage == InputDiffusionStage::Stage1_Part1)
                return juce::roundToInt(static_cast<Float>(0.004771) * fs);
            if (diffusionStage == InputDiffusionStage::Stage1_Part2)
                return juce::roundToInt(static_cast<Float>(0.003595) * fs);
            if (diffusionStage == InputDiffusionStage::Stage2_Part1)
                return juce::roundToInt(static_cast<Float>(0.012735) * fs);
            if (diffusionStage == InputDiffusionStage::Stage2_Part2)
                return juce::roundToInt(static_cast<Float>(0.009307) * fs);
    ...
    public:
        ...
        void prepare(FloatType sampleRate)
        {
            ...
            {
                stage.reset();
                stage.setDelayLength(
                    Config::getInputDiffusionTimeSamples(static_cast<Config::InputDiffusionStage>(i),
                                                         sampleRate));
            }
        }

        auto &getStage(Config::InputDiffusionStage stage)
        {
    ...
    private:
        std::array<Lattice<FloatType, Config::inputDiffusionStageLatticeLength>, static_cast<size_t>(Config::InputDiffusionStage::NumStages)> stages;
